...
network:
  ...
  tx_proto: "tcp" # Carrier for outbound path (client -> server)
  rx_proto: "udp" # Carrier for return path (server -> client), TCP-over-UDP
  ipv4:
...
network:
  ...
  tx_proto: "udp" # Return path carrier (server -> client), TCP-over-UDP
